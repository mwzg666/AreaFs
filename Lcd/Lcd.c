...
BOOL LcdCmd(BYTE Cmd, WORD Addr, BYTE *Data, BYTE Length)
{
    ...
    DebugMsg(".");
    Uart1Send(SendBuf1,(u8)(Length+6));
...
char *PopChar()
{
    ...
    sprintf(ret,(char*)pFrame->Data);
    //memcpy(&ret, &pFrame->Data[DataIndex]);
    //ret = SwWord(ret);
    //DataIndex += 2;
    return ret;
}
...
{
    LcdCmd(LCD_CMD_WRITE, mp+5, (BYTE *)&Num, 2);
}
...
// 开始显示图标
void ShowIcon(WORD Mp,WORD Val)
{
    //WORD dat = SwWord(Val);
    LcdCmd(LCD_CMD_WRITE, Mp, (BYTE *)&Val, 2);
}
...
{
    BYTE dat[2] = {64,00};
    LcdCmd(LCD_CMD_WRITE, 0x0082, (BYTE *)&dat, 2);
}


void ShowLight(WORD Light)
{
    //BYTE dat[2] = {64,00};
    WORD dat =SwWord(Light);
    LcdCmd(LCD_CMD_WRITE, 0x0082, (BYTE *)&dat, 2);
}


//旋转指针
void ShowSpin(WORD degree)
{
    WORD dat = SwWord(degree); 
    LcdCmd(LCD_CMD_WRITE, REG_SPIN, (BYTE *)&dat, 2);
}

//显示变量数据
void ShowVal(WORD *val)
{
    BYTE i = 0;
    WORD dat[SCALE_NUM];
    ...
    LcdCmd(LCD_CMD_WRITE, REG_SCALE_START, (BYTE *)&dat, 22);
}
